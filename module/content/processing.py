...
class Processing(ProcessingInterface):
    def process(self, rest_client: JobRestClient,
        ...
        # DO NOT CHECK ACCESS TYPE (temporary)
        # if input_pin.access_type == AccessTypes.FTP:
        from ftplib import FTP
        from urllib.parse import urlparse

        if input_pin.access_credential is not None and 'connectionstring' in input_pin.access_credential:
            url = urlparse(input_pin.access_credential['connectionstring'])
        elif 'connectionstring' in input_access_details:
            url = urlparse(input_access_details['connectionstring'])
        else:
            raise ValueError('missing "connectionstring" in both config and input token')
